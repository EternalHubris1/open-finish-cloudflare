...
import { DailyActivityChart, DailyActivityPoint } from '@/components/daily-activity-chart';
...
export default function Streaks() {
  ...
  const rangeEnd = useMemo(() => addDays(rangeStart, WEEKS_TO_SHOW * 7 - 1), [rangeStart]);
  ...
  const isLoading = activitiesQuery.isLoading || streaksQuery.isLoading || calendarQuery.isLoading;
  const isError = activitiesQuery.isError || streaksQuery.isError || calendarQuery.isError;

  if (isLoading) {
    return (
      <div className="mx-auto max-w-6xl space-y-6 px-4 py-6 md:p-8">
        <Skeleton className="h-14 w-64 rounded-3xl bg-white/5" />
        {[0, 1, 2].map((item) => <Skeleton key={item} className="h-72 rounded-3xl bg-white/5" />)}
      </div>
    );
  }
    ...
    return (
      <div className="mx-auto flex min-h-screen max-w-xl items-center px-4 text-center">
        <div className="w-full rounded-3xl border border-red-500/20 bg-red-950/20 p-10">
          ...
          <h1 className="mb-2 text-2xl font-bold text-white">Couldn’t load streaks</h1>
          <p className="mb-6 text-sm text-white/50">Your activity is still saved. Check the connection and try again.</p>
          <Button
            onClick={() => Promise.all([activitiesQuery.refetch(), streaksQuery.refetch(), calendarQuery.refetch()])}
  ...
  return (
    <div className="relative z-10 mx-auto min-h-screen max-w-6xl space-y-8 px-4 py-6 pb-28 md:p-8 md:pb-20">
      <div className="border-b border-white/10 pb-6">
        ...
        <h1 className="text-4xl font-bold tracking-tight text-white md:text-5xl">Streaks</h1>
        <p className="mt-3 max-w-2xl text-sm leading-relaxed text-white/45">
          Each activity has its own rhythm. A day counts when you log anything for that activity — you never have to finish everything at once.
        </p>
      </div>
      ...
      {activities.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-white/10 bg-white/[0.025] p-12 text-center">
          ...
          <h2 className="mb-2 text-xl font-bold text-white">Create an activity to begin</h2>
          <p className="mb-6 text-sm text-white/40">Its daily diagram and streak will appear here.</p>
        ...
        <div className="space-y-6">
          {activities.map((activity) => {
            const streak = streaks.find((item) => item.activityId === activity.id);
              ...
              >
                <div className="flex flex-col gap-5 border-b border-white/5 p-6 md:flex-row md:items-center md:justify-between md:p-8">
                  <div className="flex items-center gap-4">
                    <span className="h-12 w-1.5 rounded-full" style={{ backgroundColor: activity.color }} />
                    <div>
                      <Link href={`/activities/${activity.id}`} className="text-2xl font-bold text-white transition-colors hover:text-red-400">
                        {activity.name}
                      </Link>
                      <p className="mt-1 text-[10px] font-bold uppercase tracking-widest text-white/30">{activity.category}</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-3 md:flex">
                    <div className="rounded-2xl border border-orange-500/20 bg-orange-500/10 px-4 py-3 text-center md:min-w-28">
                      ...
                      <p className="text-[9px] font-bold uppercase tracking-wider text-white/30">current</p>
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-center md:min-w-28">
                      ...
                      <p className="text-[9px] font-bold uppercase tracking-wider text-white/30">best</p>
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-center md:min-w-28">
                      <p className="text-2xl font-bold text-white/80">{activeDays}</p>
