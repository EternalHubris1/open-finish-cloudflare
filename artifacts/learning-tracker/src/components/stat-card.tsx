  ...
  return (
    <div
      className={cn(
        ...
        variant === 'primary' && 'bg-gradient-to-br from-red-900/40 to-red-950/40 border-red-500/30 shadow-2xl',
        variant === 'accent' && 'bg-gradient-to-br from-orange-900/30 to-red-900/30 border-orange-500/20 shadow-2xl',
        variant === 'default' && 'bg-[rgba(15,15,20,0.85)] border-white/10 hover:border-white/20',
        className
      )}
    ...
    >
      <div className="flex items-start justify-between relative z-10">
        <div className="flex-1">
          <p className="text-[10px] font-bold mb-2 text-white/40 uppercase tracking-widest">
            {label}
          </p>
          <p className="text-3xl font-bold tracking-tight text-white">{value}</p>
          {trend && (
            <p className="text-[11px] mt-2 text-white/50 uppercase tracking-wider font-semibold">
              {trend}
            </p>
          )}
        </div>
        <div
          className={cn(
            ...
            variant === 'default' && 'bg-white/5 text-white/40 group-hover:text-white transition-colors'
          )}
        >
